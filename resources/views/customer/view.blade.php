...
<body>
    ...
    <nav>
        ...
        <a href="{{ url('/view') }}" id="change">Pending</a>
    ...
    <form action="{{ url('/view') }}" method="post" id="form">
        @csrf
        <h1 align="center">Pending View Status</h1>
        <table align="center" border="5px">
            <tr>
    ...
    @if(isset($records))
    <h2 style="text-align:center;">Pending Records for {{ $company }}</h2>
    @if($records->isEmpty())
    ...
    <table align="center" border="1" cellpadding="10" id="table">
        ...
        <tr id="tr">
            ...
            <th id="th">Contact</th>
            <th id="th">Company</th>
            <th id="th">Given Date</th>

            <!-- Add other columns as needed -->
        </tr>
        @foreach($records as $record)
        <tr id="tr">
            <td id="td">{{ $record->u_name }}</td>
            <td id="td">{{ $record->dia_no }}</td>
            <td id="td">{{ $record->contact }}</td>
            <td id="td">{{ $record->company }}</td>
            <td id="td">{{ $record->given }}</td>
